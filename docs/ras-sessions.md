...
Путь к платформе зашит как `8.3.27.2130` в двух местах —
`.devcontainer/devcontainer.json` (`mounts`) и `.devcontainer/post-create.sh`
(`RAC_BIN`). При обновлении платформы 1С поправьте версию в обоих файлах и
