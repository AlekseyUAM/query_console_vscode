...
describe('parseQuery 6.15.7 — блок построителя {ГДЕ}: условия-выражения и стоп на «{»', () => {
  ...
  // `.*` при наличии КАК конструктор бросает (корпус: КонвертацияОбъектов bsl_5),
  // без КАК — сохраняет (СервисКриптографии bsl_3).
  it('{ГДЕ}: «.*» с псевдонимом отбрасывается, без псевдонима сохраняется', () => {
