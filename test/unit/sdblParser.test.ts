...
describe('parseQuery — отрицательные ветви распознавания', () => {
  ...
  it('простое поле: неизвестный псевдоним → выражение', () => {
  ...
  it('поле без ИЗ: parseOneField доходит до eof перед ошибкой ИЗ', () => {
    expect(() => parseQuery('ВЫБРАТЬ Т.А')).toThrow(/ожидалось ключевое слово «ИЗ»/);
  });
});
...
describe('parseDocument — переписывание псевдонимов колонок', () => {
  it('NULL-ячейка отбрасывается, лишнее поле без псевдонима колонки сохраняется', () => {
    // Участник 0 задаёт 2 колонки; участник 1 имеет NULL в первой и доп. поле без колонки.
    const text =
    ...
    // Участник 1: NULL отброшен → осталось 2 поля (Наименование + лишнее ИНН без колонки).
    expect(doc.members[1].model.fields.length).toBe(2);
  });
